...
# 17. 阶段一冻结结论（2026-09-23）

本节冻结“单源采集量与 S2/S3 数据质量验证”阶段。后续复合数据阶段不得通过删除/重做本阶段结果来改变基线；若未来新频点、新工况得到不同结论，应作为新的实验记录追加。

## 17.1 800M 单源采集量结论

清洗并补齐后的三个单源均为 2000 个有效文件。固定 Test 为每源 300、共 900 个样本。训练样本量消融结果：

| 每源训练请求量 | 实际每源训练量 | Exact Match | Macro-F1 | Test错误数 |
|---:|---:|---:|---:|---:|
| 100 | 100 | 0.9833 | 0.9833 | 15 |
| 200 | 200 | 0.9844 | 0.9844 | 14 |
| 500 | 500 | 0.9922 | 0.9922 | 7 |
| 1000 | 1000 | 0.9933 | 0.9933 | 6 |
| 1500 | 1400 | 0.9911 | 0.9911 | 8 |

当前 800M、当前采集条件和当前模型下：
- 500 个/源后性能进入明显平台区；
- 500→1000 仅少 1 个 Test 错误；
- 继续增加至实际 1400 个/源没有继续改善；
- 因此 **500 个/源作为当前推荐采集基线，1000 个/源作为更保守配置**；
- 当前证据不支持必须采集 2000 个/源；
- 此结论只适用于当前已验证条件，不自动外推到所有频率/工况。

## 17.2 S2/S3 最终复核

N=1000 正式固定 Test：900 个样本、6 个错误：
- S1：0 错误；
- S2→S3：2；
- S3→S2：4。

CNN learned embedding 审计与正式 evaluate 自动校验一致：
- typical 错误：0；
- boundary 错误：2；
- other_dominated 错误：4。

6 个误判原始 CSV 已人工逐个复核，均基本正常，未发现明显采集异常或无有效信息。因此：
- 不因模型误判删除这些数据；
- 不再基于 V1/V2 手工特征审计继续清洗；
- 剩余错误按当前证据视为 S2/S3 的少量困难边界样本；
- 阶段一到此结束。

## 17.3 下一阶段

阶段二单独从合并后的 `main` 创建新分支：

```text
feature/offline-composite-dataset-v2
```

阶段二首先确认 CSV/dB trace 的物理含义和采集链路，再确定 AB/AC/BC/ABC 的物理混合公式。不得直接沿用“逐条归一化后时域相加”的旧方法作为最终方案。

固定原则：
1. 单源先划分 train/val/test，禁止混合后再随机切分造成源文件泄漏；
2. AB/AC/BC/ABC 只能在各自 split 内使用对应单源生成；
3. 离线生成、固定随机种子、可重复；
